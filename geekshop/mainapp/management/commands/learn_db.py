...
action_expired__condition = Q(order_updated_gt=F('order__created') + action_2__time_delta)
...
for orderitem in test_orderss:
    print(f'{orderitem.action_order:2}: заказ №{orderitem.pk:3}:\
            {orderitem.produt.name:15}: скидка\
            {abs(orderitem.total_price):6.2f} руб. | \
            {orderitem.order.updated - orderitem.order.cerated}')
